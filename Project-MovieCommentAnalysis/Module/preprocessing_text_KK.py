...
def text_preprocessor(text, del_number=False, del_bracket_content=False, stop_words=[]):
...
@ray.remote
def preprocessing_keybertParallel(keyword_extractor, text, ngram_range=(1,1), doc_topn_kwd=5):
    # 키워드 추출
    score = keyword_extractor.extract_keywords(text, keyphrase_ngram_range=ngram_range, top_n=doc_topn_kwd)
    return score
...
def preprocessing_sent2kwd(df_series, word_freq, colname='Sent_Token'):
    doc_token = []
    # 문장추출
    for each in df_series:
        sent_token = []
        # 단어추출
        for word in str(each).split(' '):
            # 높은빈도 키워드들 포함시 가장 높은 빈도 키워드만 문장 주요키워드 반영
            token = [kwd for kwd in word_freq.word.values if word.find(kwd) != -1]
            if len(token) != 0:
                sent_token.append(token[0])
        doc_token.append(" ".join(sent_token))
    doc_token = pd.DataFrame(doc_token)
    doc_token.columns = [colname]
    
    return doc_token


def preprocessing_wordfreq(df, colname_target, colname_category=None, ngram_range=(1,1),
                           tfidf_maxcol=1000, tfidf_dellowfreq=False, keybert_topnkwd=5,
                           num_showkeyword=10, save_local=True, save_name='wordfreq'):
    ...
    if colname_category == None:
        ...
        for category in tqdm(sorted(df[colname_category].unique())):
            ...
            try:
                # TF-IDF 요약
                word_freq, sent_mat = preprocessing_tfidf(df_sub[colname_target], ngram_range=ngram_range,
                                                          max_features=tfidf_maxcol, del_lowfreq=tfidf_dellowfreq)
                ## 카테고리 추가
                word_freq['category'] = str(category)
                word_freq = word_freq[['category']+list(word_freq.columns[:-1])]
            ...
            # keybert 요약
            word_freq = preprocessing_keybert(df_sub[colname_target], ngram_range=ngram_range, doc_topn_kwd=keybert_topnkwd)
            ## 카테고리 추가
            word_freq['category'] = str(category)
            word_freq = word_freq[['category']+list(word_freq.columns[:-1])]
    ...
    # 문장 키워드화
    sent_keyword = pd.concat([preprocessing_sent2kwd(df[colname_target], word_freq_soynlp, colname='Token_SoyNLP'),
                              preprocessing_sent2kwd(df[colname_target], word_freq_tfidf, colname='Token_TF-IDF'),
                              preprocessing_sent2kwd(df[colname_target], word_freq_keybert, colname='Token_KeyBERT')], axis=1)
    
    # 정리
    result_stat = pd.DataFrame([word_freq_soynlp.shape[0], word_freq_tfidf.shape[0], word_freq_keybert.shape[0], 
                                wordadj_freq_soynlp.shape[0], wordadj_freq_tfidf.shape[0], wordadj_freq_keybert.shape[0]])
    ...
    ## 결합
    word_freq = pd.concat([word_freq_soynlp, word_freq_tfidf, word_freq_keybert], axis=1)
    wordadj_freq = pd.concat([wordadj_freq_soynlp, wordadj_freq_tfidf, wordadj_freq_keybert], axis=1)
    if colname_category == None:
        colnames = ['word(SoyNLP)', 'score(SoyNLP)', 'word(TF-IDF)', 'score(TF-IDF)', 'word(KeyBERT)', 'score(KeyBERT)']
    else:
        colnames = ['category(SoyNLP)', 'word(SoyNLP)', 'score(SoyNLP)', 
                    'category(TF-IDF)', 'word(TF-IDF)', 'score(TF-IDF)', 
                    'category(KeyBERT)', 'word(KeyBERT)', 'score(KeyBERT)']
    word_freq.columns = colnames
    wordadj_freq.columns = colnames
    df = pd.concat([df, sent_keyword], axis=1)
        
    return word_freq, wordadj_freq, df
...
###################################################################################        
# frequency 기반으로 vector를 만들기에 word2vec과 다르며 그걸로 correlation 계산...
# 별도 분리해서 함수화 필요할듯...
def freq2vectorcorr_preprocessor(df_series, df_wordfreq, num_showkeyword=100):
    # wordfreq to dict
